...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // Different wallets return different arrays
      let signature: [string, string];
      // if (walletType === "argentx") {
      //   signature = [rawSignature[2], rawSignature[3]];
      // } else if (walletType === "braavos") {
      ...
      const sigArray = rawSignature as string[];

if (walletType === "argentx") {
  signature = [sigArray[2], sigArray[3]];
} else if (walletType === "braavos") {
  signature = [sigArray[1], sigArray[2]];
} else {
  throw new Error("Unsupported wallet type");
}


      await verifySignature(address, signature, nonce, walletType, locale);
    } catch (err) {
      console.error("Login failed:", err);
      setLocalError(err instanceof Error ? err.message : "Login failed");
    }
